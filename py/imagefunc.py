...
def cv22ski(cv2_image:np.ndarray) -> np.ndarray:
    return cv2_image.astype(np.float32) / 255.0

def ski2cv2(ski:np.ndarray) -> np.ndarray:
    return np.clip(ski * 255.0, 0, 255).astype(np.uint8)
...
def chop_image_v2(background_image:Image, layer_image:Image, blend_mode:str, opacity:int) -> Image:
    ...
    source_prepped = np.asarray(layer_image.convert('RGBA'), dtype=np.float32)
...
def adjust_levels(image:Image, input_black:int=0, input_white:int=255, midtones:float=1.0,
    ...
    img = pil2cv2(image).astype(np.float32)
...
def pixel_spread(image:Image, mask:Image) -> Image:
    ...
    i_dup = copy.deepcopy(i1.cpu().numpy().astype(np.float32))
    a_dup = copy.deepcopy(pil2tensor(mask).cpu().numpy().astype(np.float32))
    fg = copy.deepcopy(i1.cpu().numpy().astype(np.float32))
...
def mask_edge_detail(image:torch.Tensor, mask:torch.Tensor, detail_range:int=8, black_point:float=0.01, white_point:float=0.99) -> torch.Tensor:
    ...
    if not bool(d % 2):
        d += 1
    i_dup = copy.deepcopy(image.cpu().numpy().astype(np.float32))
    a_dup = copy.deepcopy(mask.cpu().numpy().astype(np.float32))
    for index, img in enumerate(i_dup):
